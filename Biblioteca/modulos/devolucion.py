...
def mostrar_libros(libros: list) -> None:
    ...
    for i, libro in enumerate(libros, 1):
        print(
            f"{i}. {libro['title']} - Fecha de préstamo: {libro['fecha_de_prestamo']}"
        )
    ...
    while True:
        try:
            ...
            if num_libro >= 1 and num_libro <= len(libros):
                ...
                print(
                    "Error: El número no corresponde a ningún libro. Inténtelo nuevamente."
                )
        except ValueError:
...
def gestionar_review(libro_devuelto: dict) -> None:
    ...
    if op == "s":
        ...
        if review:
            try:
                with open(
                    "Biblioteca/JSON/reviews.json", "r", encoding="utf-8"
                ) as leer_reviews:
                    try:
            ...
            try:
                with open(
                    "Biblioteca/JSON/reviews.json", "w", encoding="utf-8"
                ) as escribir_reviews:
    ...
    if prestamo:
        ...
        if libros_prestados:
            ...
            if num_libro:
                ...
                menues.clear_screen()
                print(
                    f"El libro '{libro_devuelto['title']}' ha sido devuelto exitosamente."
                )
